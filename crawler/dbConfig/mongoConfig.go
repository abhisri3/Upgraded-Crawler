...
func InsertData(insuranceData model.InsuranceData) {
	...
	filter := bson.M{"url": insuranceData.Url}
	
	notFound := dataCollection.FindOne(context.Background(),filter).Decode(&olddata)
	if notFound != nil{
		...
		fmt.Println("found " )
		insuranceData.Id = olddata.Id
		filterID := bson.M{"_id": olddata.Id}
		dataCollection.ReplaceOne(context.Background(),filterID,insuranceData)
